...
- ``sgx.preheat_enclave_size = "1"`` -- pre-fault all enclave pages during
  enclave initialization when ``sgx.edmm_enable_heap = false``. This shifts the
  overhead of page faults on non-present enclave pages from runtime to enclave
  startup time. Using this option makes sense only if the whole enclave memory
  fits into :term:`EPC`.
