            ...
            if (sub_regions[i].size == sizeof(uint8_t)) {
                ...
                *value = (ssize_t)(*((uint64_t*)sub_regions[i].encl_addr));
                ...
                log_error("Invalid deep-copy syntax (deep-copy sub-entry '%s' must be of "
                          "legitimate size: 1, 2, 4 or 8 bytes)", sub_regions[i].name);
                return -EINVAL;
            }
...
static int calculate_boolean_expr(struct sub_region* sub_regions, int sub_regions_cnt, char* expr,
    ...
    if (compar_len != 2 || (memcmp(compar, "==", 2) && memcmp(compar, "!=", 2) &&
        ...
        log_error("Invalid deep-copy syntax (the only supported comparators are '==', '!=', "
                  "'&=' and '!=' but they are not found in expression '%s')", expr);
        return -EINVAL;
    }

    if (*cur != '\0' || !token1_len || !token2_len) {
        log_error("Invalid deep-copy syntax (cannot parse expression '%s')", expr);
        return -EINVAL;
    }
    ...
    while (mem_region_idx < mem_regions_cnt) {
        ...
        for (int i = 0; i < toml_array_nelem(cur_mem_region->toml_array); i++) {
            ...
            if (!cached) {
                ...
                /* check if "size" is static (and positive) -> cache it */
                new_cached->size = -1;
            ...
            cur_encl_addr += cur_sub_region->type == COPY_PTR_ENCLAVE ? sizeof(void*) :
                                 (uintptr_t)cur_sub_region->size;
        }
...
static void copy_sub_regions_to_enclave(struct sub_region* sub_regions, int sub_regions_cnt,
                                        unsigned int cmd) {
    for (int i = 0; i < sub_regions_cnt; i++) {
        size_t size = sub_regions[i].size;

        /* FIXME: workaround for bug in IOCTL 0xc0106492: copy more memory than specified */
        if (cmd == 0xc0106492 && sub_regions_cnt == 10 && i == 9 && size == 84) {
            size = 116;
        }

        if (size <= 0 || !sub_regions[i].encl_addr)
            continue;

        if (sub_regions[i].type == COPY_IN_ENCLAVE || sub_regions[i].type == COPY_INOUT_ENCLAVE)
            memcpy(sub_regions[i].encl_addr, sub_regions[i].untrusted_addr, size);
    }
}
...
int _DkDeviceIoControl(PAL_HANDLE handle, unsigned int cmd, uint64_t arg) {
    ...
    if (ret < 0) {
        if (ret != -EFAULT) {
            log_error("Invalid struct format of allowed ioctl '%u' in manifest", cmd);
        }
        return unix_to_pal_error(ret);
    }
    ...
    copy_sub_regions_to_enclave(sub_regions, sub_regions_cnt, cmd);
